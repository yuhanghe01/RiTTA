...
class Cnn14_DecisionLevelMax(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        ...
        amin = 1e-10
        top_db = None
        self.interpolate_ratio = 6     # Downsampled ratio
        ...
        self.fc1 = nn.Linear(2048, 2048, bias=True)
        self.fc_audioset_audiollm = nn.Linear(2048, classes_num, bias=True)
    ...
    def init_weight(self):
        init_bn(self.bn0)
        init_layer(self.fc1)
        init_layer(self.fc_audioset_audiollm)
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x_pooled = self.avg_pool(x_transpose)
        x = x_pooled.transpose(1, 2)

        segmentwise_logits = self.fc_audioset_audiollm(x)
...
def main():
    ...
    model = Cnn14_DecisionLevelMax(sample_rate, window_size, hop_size, mel_bins, fmin, fmax, classes_num)
    model.to(device)
    model.train()

    batch_size = 32
    time_length = 10 * sample_rate
    data = torch.randn(batch_size, time_length)
