...
def get_all_data():
    ...
    keys_list = ['Count', 'Compositionality', 'Temporal_Order', 'Spatial_Distance']

    for key_tmp in keys_list:
        sub_key_list = data_dict[key_tmp].keys()
        for sub_key in sub_key_list:
            for data_tmp in data_dict[key_tmp][sub_key]:
                reference_audio = data_tmp['reference_audio']
                text = data_tmp['text_prompt']
                all_data.append({'text': text, 'text_basename': reference_audio[0]})

    return all_data, 'gen_data_test/'
...
def main():
    ...
    all_data, save_dir = get_all_data()
    for data_tmp in all_data:
        print('process text:', data_tmp['text'])
        text_prompts = [data_tmp['text']]
        text_basename = data_tmp['text_basename']
        pred_audio_save_basename = text_basename.replace('.wav', '_tango.wav')
        with torch.no_grad():
            latents = model.inference(text_prompts, scheduler, num_steps, guidance, num_samples, disable_progress=True)
            mel = vae.decode_first_stage(latents)
            wave = vae.decode_to_waveform(mel)
            wave = wave[0]
            sf.write(os.path.join(save_dir, pred_audio_save_basename), wave, samplerate=16000)
